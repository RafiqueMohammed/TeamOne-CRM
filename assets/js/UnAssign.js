   ...
   $.getJSON('api/Assign/Customer',function(response){
    if(response.status=="ok"){
        ...
        $.each(response.data,function(key,value){
            ...
            <td>"+value.mobile+"</td><td>"+value.customer_location+"</td><td><button class='btn btn-primary btn-sm' onclick='return LoadPage(\"AssignTechnician?id="+value.cust_id+"\");' href='AssignTechnician?id="+value.cust_id+"'>View&nbsp;&nbsp;<i class='fa fa-arrow-circle-right'></i></button></td></tr>"
        });
        $("#customer_list_for_technician_assignment tbody").prepend(row);
    }else if(response.status=="no"){
        console.log("error");
        $("#customer_list_for_technician_assignment tbody").html("<tr><td colspan='6' class='alert alert-info center'><i class='clip-info'></i>&nbsp;&nbsp;&nbsp;No Services to be Assigned</td></tr>");
    }
