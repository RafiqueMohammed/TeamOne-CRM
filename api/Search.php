...
$app->get("/Search/Residential/:str", function ($str) use ($app) {
    global $DB;
    $check = trim(str_replace(array(" ", "-", "+"), "", $str));
    $qry = $DB->query("SELECT * FROM `" . TAB_CUSTOMER . "` WHERE `mobile` = '$check' AND `account_type`='r' "); //OR `alternate_mobile` LIKE '%$str%'

    if ($qry->num_rows > 0) {
        $result = array("status" => "ok");
        $result['total_account'] = $qry->num_rows;
        while ($info = $qry->fetch_assoc()) {
            $result["data"][] = $info;
        }
    } else {
        $result = array("status" => "no", "result" => "No customer found. Try something else..", "total_account" => 0);
    }
    $app->response->body(json_encode($result));
});
$app->get("/Search/Commercial/:str", function ($str) use ($app) {
    global $DB;
    $check = trim(str_replace(array(" ", "-", "+"), "", $str));
    $qry = $DB->query("SELECT * FROM `" . TAB_CUSTOMER . "` WHERE `mobile` = '$check'  AND `account_type`='c'"); //OR `alternate_mobile` LIKE '%$str%'
