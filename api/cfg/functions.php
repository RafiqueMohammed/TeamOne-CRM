...
function generateTicketID($type, $cust_id, $service_id)
{

    return substr(strtoupper($type), 0, 3) . "-" . $cust_id . "-" . date("ymd") . $service_id;
}
...
function SendSMS($to, $msg)
{
    ...
    if (empty($to) && empty($msg)) {
        ...
        $response['result'] = "Phone Number/Message cannot be empty";
    } else {
        /*$url = "http://slogin.ovmedia.in/API/WebSMS/Http/v1.0a/index.php?username=teamone&password=12345&sender=TEAMAC&to=$to&message=" . urlencode($msg) . "&reqid=1&format=json&route_id=22";
        ...
        if (isset($response['msg_id']) && !empty($response['msg_id'])) {
            ...
            $response['result'] = "Successfully Sent";
        } else {
            $response['success'] = false;
            $response['result'] = "SMS failed to send!";
            $response['mobile'] = $to;
        }*/
        $response['success'] = true;
        $response['result'] = "Successfully Sent";
...
function NotifySMS_ori($assign_id)
{
    $res = array();
    $error = 0;
    global $DB;
