...
{
    global $DB; if (isset($_POST['registration']) && !empty($_POST['registration']))
    {
        $fname = (isset($_POST['firstname'])) ? $_POST['firstname'] : ""; $lname = (isset
            ($_POST['lastname'])) ? $_POST['lastname'] : ""; $acc_type = (isset($_POST['acc_type'])) ?
            $_POST['acc_type'] : "r"; $org = (isset($_POST['org_name'])) ? $_POST['org_name'] :
            ""; $mob1 = (isset($_POST['mobile1'])) ? $_POST['mobile1'] : ""; $mob2 = (isset
            ($_POST['mobile2'])) ? $_POST['mobile2'] : ""; $email = (isset($_POST['email'])) ?
            $_POST['email'] : ""; $landline = (isset($_POST['landline'])) ? $_POST['landline'] :
            ""; $address = (isset($_POST['address'])) ? $_POST['address'] : ""; $pincode = (isset
            ($_POST['pincode'])) ? $_POST['pincode'] : ""; $city = (isset($_POST['city'])) ?
            $_POST['city'] : ""; $landmark = (isset($_POST['landmark'])) ? $_POST['landmark'] :
            ""; $location = (isset($_POST['location'])) ? $_POST['location'] : ""; $alt_contacts =
            (isset($_POST['alternate_contact'])) ? $_POST['alternate_contact'] : ""; $mode_of_com =
            (isset($_POST['communication_type'])) ? $_POST['communication_type'] : ""; $referred =
            (isset($_POST['referred_by'])) ? $_POST['referred_by'] : ""; $other = (isset($_POST['reffered_other'])) ?
            $_POST['reffered_other'] : ""; $dob = (isset($_POST['dob'])) ? $_POST['dob'] :
            ""; $reference = ""; if ($referred == "other" || $referred == "exist_cust")
        {
            $reference = $other; }
    else
    {
    ...
    $remarks = (isset($_POST['remarks'])) ? $_POST['remarks'] : ""; $DB->query("INSERT INTO `" .
...
{
global $DB; $qry = $DB->query("SELECT * FROM `" . TAB_CUSTOMER . "`"); if ($qry->
num_rows > 0)
{
...
while ($data = $qry->fetch_assoc())
{
    ...
    $data["created_on"] = ConvertToIST($data['created_on']); $result["data"][] = $data; }
...
{
global $DB; $qry = $DB->query("SELECT * FROM `" . TAB_CUSTOMER .
"`  WHERE `cust_id` = '{$id}'"); if ($qry->num_rows > 0)
{
$data = $qry->fetch_assoc(); $result = $data; $result['status'] = "ok"; $originalDate =
    $data['created_on']; $dob_raw = $data['dob']; if ($dob_raw == "0000-00-00" || $dob_raw == null)
{
    $dob_convert = ""; }
else
{
...
$newDate = date("d-m-Y", strtotime($originalDate)); $result['date'] = $newDate;
    $result['dob'] = $dob_convert; $pQry = $DB->query("SELECT * FROM `" .
    TAB_LOCALITY . "` WHERE `pincode`='{$data['pincode']}'"); $result['landmark_info'] =
    array(); if ($pQry->num_rows > 0)
{
...
$acQry = $DB->query("SELECT * FROM `" . TAB_CUSTOMER_AC .
    ...
    "` as loc WHERE ac_type.`ac_type_id`=cust_ac.`ac_type` AND cust_ac.`make`=make.`make_id` AND loc.ac_location_id=cust_ac.ac_location
        AND ton.tonnage_id=cust_ac.capacity AND cust_ac.`cust_id`='{$id}'"); $result['ac_data'] =
    array(); if ($acQry->num_rows > 0)
{
    $result['ac_data']["status"] = "ok"; while ($acInfo = $acQry->fetch_assoc())
    {
        $result['ac_data'][] = $acInfo; }
}
else
{
    $result['ac_data']["status"] = "no"; $result['ac_data']["result"] =
        " - No AC Added Yet -"; }
...
{
global $DB; $result = array(); $fname = $DB->real_escape_string($app->request->
put('fname')); $lname = $DB->real_escape_string($app->request->put('lname')); $org =
$DB->real_escape_string($app->request->put('org')); $mob1 = $DB->
real_escape_string($app->request->put('mobile1')); $mob2 = $DB->
real_escape_string($app->request->put('mobile2')); $email = $DB->
real_escape_string($app->request->put('email')); $landline = $DB->
real_escape_string($app->request->put('landline')); $address = $DB->
real_escape_string($app->request->put('address')); $pincode = $DB->
real_escape_string($app->request->put('pincode')); $city = $DB->
real_escape_string($app->request->put('city')); $landmark = $DB->
real_escape_string($app->request->put('landmark')); $location = $DB->
real_escape_string($app->request->put('location')); $alt_contacts = $DB->
real_escape_string($app->request->put('alt_contacts')); $mode_of_com = $DB->
real_escape_string($app->request->put('communication')); $remarks = $DB->
real_escape_string($app->request->put('remarks')); $dob = $DB->
real_escape_string($app->request->put('dob')); $newdate = date("Y-m-d",
strtotime($dob)); $DB->query("UPDATE `" . TAB_CUSTOMER . "` SET `first_name`='{$fname}',`last_name`='{$lname}',`email`='{$email}',`organisation`='{$org}',`mobile`='{$mob1}',
        ...
        `landmark`='{$landmark}',`location`='{$location}',`pincode`='{$pincode}',`city`='{$city}',`mode_of_contact`='{$mode_of_com}',
         `remarks`='{$remarks}' WHERE `cust_id`='{$id}'"); if ($DB->
affected_rows > 0)
{
...
{
$result = array("status" => "ok", "result" => "You deleted info for $id"); $app->
...
global $DB; $result = array(); if ($id > 0)
{

$qry = $app->request->post("row"); $app->response['Content-Type'] = 'text/html';
    $insert = 0; $total = count($qry); for ($i = 0; $i < $total; $i++)
{
    $ac_type = $DB->real_escape_string($qry[$i]["ac_type"]);
        //$model_no = $DB->real_escape_string($qry[$i]["model_no"]);
        $make = $DB->real_escape_string($qry[$i]["make"]); $idu_model_no = $DB->
        real_escape_string($qry[$i]["idu_model_no"]); $idu_serial_no = $DB->
        real_escape_string($qry[$i]["idu_serial_no"]); $odu_model_no = $DB->
        real_escape_string($qry[$i]["odu_model_no"]); $odu_serial_no = $DB->
        real_escape_string($qry[$i]["odu_serial_no"]); $location = $DB->
        real_escape_string($qry[$i]["location"]); $remarks = $DB->real_escape_string($qry[$i]["remarks"]);
        $capacity = $DB->real_escape_string($qry[$i]["ton"]); $DB->query("INSERT INTO `" .
        TAB_CUSTOMER_AC . "`(`cust_id`,`make`,`ac_type`
            ...
            '$idu_model_no','$idu_serial_no','$location','$remarks')"); if ($DB->
        affected_rows > 0)
    {
        $insert++; }
...
global $DB; $acQry = $DB->query("SELECT * FROM `" . TAB_CUSTOMER_AC .
...
if ($acQry->num_rows > 0)
{
$result["status"] = "ok"; $acInfo = $acQry->fetch_assoc(); $result['data'][] = $acInfo; }
else
{
$result["status"] = "no"; $result["result"] = "No Such AC Found"; }
...
{
global $DB; $result = array(); $result = array("status" => "ok", "result" =>
    "You updated AC $id info for $ac_id"); $app->response->body(json_encode($result)); }
);

$app->delete("/Customer/:id/AC/:ac_id", function ($id, $ac_id)use ($app)
{
global $DB; $result = array(); $DB->query("DELETE FROM `" . TAB_CUSTOMER_AC .
"` WHERE `ac_id`='$ac_id' and `cust_id`='$id'"); if ($DB->affected_rows > 0)
{
...
{
global $DB; $result = array(); $qry = $DB->query("SELECT * FROM `" .
...
$result['status'] = "ok"; while ($data = $qry->fetch_assoc())
{
    $result["data"][] = $data; }
}
else
...
{
global $DB; $qry = $DB->query("SELECT * FROM `" . TAB_INSTALL .
"` WHERE `enabled`='1'"); if ($qry->num_rows > 0)
{
$result = array("status" => "ok"); while ($info = $qry->fetch_assoc())
{
    $result['data'][] = $info; }
}
else
...
{
global $DB; $qry = $DB->query("SELECT * FROM `" . TAB_CUSTOMER_AC . "` as cad
    INNER JOIN (SELECT `install_id`, `cust_id` , `ac_id` as ins_ac_id, `install_type`, `install_date`, `no_of_service`, `remarks` as install_remarks, `enabled`, `created_on` FROM `" .
...
ThrowError($DB->error); if ($qry->num_rows > 0)
{
$result = array("status" => "ok"); while ($info = $qry->fetch_assoc())
{
    $info['install_date'] = ConvertToIST($info['install_date']); $info['created_on'] =
        ConvertToIST($info['created_on']); $result['data'][] = $info; }
}
else
{
$result = array("status" => "no", "result" => "No Installation added yet"); }
...
global $DB; if ($cid > 0)
{
$ac_id = $app->request->post("ac_id"); $ins_type = $app->request->post("ins_type");
    $no_of_service = $app->request->post("no_of_service"); $ins_date = $app->
    request->post("ins_date"); $ins_date = date("Y-m-d", strtotime($ins_date)); $remarks =
    $app->request->post("remarks"); if (!empty($ac_id) && !empty($ins_type) && !
    empty($no_of_service) && !empty($ins_date))
{
    $qry = $DB->query("SELECT `install_id` FROM `" . TAB_INSTALL .
        "` WHERE `cust_id`='$cid' AND `ac_id`='$ac_id'") or ThrowError($DB->error); if ($qry->
        num_rows > 0)
    {
...
else
{
    $DB->query("INSERT INTO `" . TAB_INSTALL .
        "`(`cust_id`, `ac_id`, `install_type`, `install_date`, `no_of_service`,`remarks`)
        VALUES('$cid','$ac_id','$ins_type','$ins_date','$no_of_service','$remarks')") or
        ThrowError($DB->error); if ($DB->affected_rows > 0)
    {
        $result['status'] = "ok"; $result['result'] = "Successfully Added"; $app->
            status(201); $app->response->body(json_encode($result)); }
else
{
...
{
global $DB; $DB->query("DELETE FROM `" . TAB_INSTALL . "` WHERE `cust_id`='$cid' and `install_id`='$id'") or
ThrowError($DB->error); if ($DB->affected_rows > 0)
{
...
{
global $DB; $qry = $DB->query("SELECT * FROM `" . TAB_AMC . "` WHERE `cust_id`='$cid'");
if ($qry->num_rows > 0)
{
$result = array("status" => "ok"); while ($info = $qry->fetch_assoc())
{
$info['activation'] = ConvertToIST($info['activation']); $info['expiration'] =
ConvertToIST($info['expiration']); $result['data'][] = $info; }
}
else
{
...
{
global $DB; $result = array(); if ($cid > 0)
{

$qry = $app->request->post("row"); $app->response['Content-Type'] = 'text/html';
$insert = 0; $total = count($qry); for ($i = 0; $i < $total; $i++)
{
$cid = $DB->real_escape_string($qry[$i]["cust_id"]); $ac_id = $DB->
real_escape_string($qry[$i]["ac_id"]); $amc_type = $DB->real_escape_string($qry[$i]["service_type"]);
$dry = $DB->real_escape_string($qry[$i]["dry_service"]); $wet = $DB->
real_escape_string($qry[$i]["wet_service"]); $no_of_service = $dry + $wet; $activation =
$DB->real_escape_string($qry[$i]["amc_datepicker"]); $expiration = date('Y-m-d',
strtotime('+1 year', strtotime($activation))); $DB->query("INSERT INTO `" .
TAB_AMC . "`(`cust_id`,`ac_id`,`amc_type`,`dry`,`wet`,`no_of_service`,`activation`,`expiration`)
...
{
global $DB; $qry = $DB->query("SELECT * FROM `" . TAB_AMC . "` WHERE `cust_id`='$cid' AND `amc_id`='$id' AND `enabled`='1'") or
ThrowError($DB->error); if ($qry->num_rows > 0)
{
$result = array("status" => "ok"); while ($info = $qry->fetch_assoc())
{
...
{
global $DB; $DB->query("DELETE FROM `" . TAB_AMC . "` WHERE `cust_id`='$cid' and `amc_id`='$id'");
if ($DB->affected_rows > 0)
{
...
{
global $DB; $qry = $DB->query("SELECT * FROM `" . TAB_OTS .
"` WHERE `enabled`='1'"); if ($qry->num_rows > 0)
{
$result = array("status" => "ok"); while ($info = $qry->fetch_assoc())
{
...
$result = array("status" => "ok"); while ($info = $qry->fetch_assoc())
{
$info['created_on'] = ConvertToIST($info['created_on']); $result['data'][] = $info; }
}
else
{
...
global $DB; $ac_id = $DB->real_escape_string($app->request->post("ac_id")); $service_type =
$DB->real_escape_string($app->request->post("service_type")); $desc = $DB->
real_escape_string($app->request->post("desc")); if ($ac_id > 0 && $service_type !=
"")
{

$DB->query("INSERT INTO `" . TAB_OTS .
"`(`cust_id`,`ac_id`,`service_type`,`description`)
         VALUES('$cid','$ac_id','$service_type','$desc')") or ThrowError($DB->
error); if ($DB->affected_rows > 0)
{
$result['status'] = "ok"; $result['result'] = "Successfully added"; $app->
status(201); $app->response->body(json_encode($result)); }
else
...
{
global $DB; $qry = $DB->query("SELECT * FROM `" . TAB_OTS . "` WHERE `cust_id`='$cid' AND `ots_id`='$id' AND `enabled`='1'");
if ($qry->num_rows > 0)
{
$result = array("status" => "ok"); while ($info = $qry->fetch_assoc())
{
...
{
global $DB; $DB->query("DELETE FROM `" . TAB_OTS . "` WHERE `cust_id`='$cid' and `ots_id`='$id'") or
ThrowError($DB->error); if ($DB->affected_rows > 0)
{
...
{
global $DB; $qry = $DB->query("SELECT * FROM `" . TAB_COMPLAINT .
"` WHERE `enabled`='1'"); if ($qry->num_rows > 0)
{
$result = array("status" => "ok"); while ($info = $qry->fetch_assoc())
{
...
global $DB; if ($cid > 0)
{
$ac_id = $app->request->post("ac_id"); $problem_type = $app->request->post("problem_type");
$problem_desc = $app->request->post("problem_desc"); if (!empty($ac_id) && !
empty($problem_type))
{
...
$DB->query("INSERT INTO `" . TAB_COMPLAINT .
    "`(`cust_id`, `ac_id`,`problem_type`, `problem_desc`)
        VALUES('$cid','$ac_id','$problem_type','$problem_desc')") or ThrowError
    ($DB->error); if ($DB->affected_rows > 0)
{
    $result['status'] = "ok"; $result['result'] = "Successfully Added"; $app->
        status(201); $app->response->body(json_encode($result)); }
else
