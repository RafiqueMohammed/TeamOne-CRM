...
$app->get("/Customer/:id", function ($id) use ($app) {
    ...
    if ($qry->num_rows > 0) {
        ...
        $originalDate =
            $data['created_on'];
        $dob_raw = $data['dob'];
        if ($dob_raw == "0000-00-00" || $dob_raw == null || empty($dob_raw)) {
            $dob_convert = "";
        } else {
...
$app->put("/Customer/:id", function ($id) use ($app) {
    ...
    $dob = ($app->request->put('dob', false) != false) ? $DB->real_escape_string($app->request->put('dob')) : "0000-00-00";
    $newdate = date("Y-m-d",
        strtotime($dob));
...
INNER JOIN (SELECT `install_id`, `cust_id` , `ac_id` as ins_ac_id, `install_type`, `preferred_date`, `no_of_service`, `remarks` as install_remarks, `enabled`, `created_on` FROM `" . TAB_INSTALL . "`) as install,(SELECT ac_type as actype,`ac_type_id` FROM `" . TAB_AC_TYPE . "`) as ac_type ,
...
WHERE install.`cust_id`='$cid' and install.`ins_ac_id` = cad.`ac_id` and cad.`make`=ac_make.`make_id`
 and acloc.`ac_location_id`=cad.`ac_location`  and ac_type.`ac_type_id` = cad.`ac_type` and ac_ton.`tonnage_id`=cad.capacity and install.`enabled`='1' ") or
    ...
    if ($qry->num_rows > 0) {
        $result = array("status" => "ok");
        while ($info = $qry->fetch_assoc()) {
            $check_assign = $DB->query("SELECT * FROM `install_service_date` as ins_serv INNER JOIN  `assignment` as assign
 ON assign.`type`='installation' AND assign.`type_id`=ins_serv.install_service_id
WHERE ins_serv.install_id='{$info['install_id']}'") or ThrowError($DB->error);

            if ($check_assign->num_rows > 0) {
                $check_data = $check_assign->fetch_assoc();
                if ($check_data['status'] == 'c') {
                    $info['assignment_status'] = "Done";
                } else {
                    $info['assignment_status'] = "Pending..";
                }
            } else {
                $info['assignment_status'] = "Technician Not Assigned";
            }
...
$app->delete("/Customer/:cid/Installations/:id", function ($cid, $id) use ($app) {
    global $DB;
    $qry=$DB->query("SELECT * FROM `".TAB_INSTALL_SERVICE."` WHERE `install_id`='{$id}' AND `type`='installation'") or ThrowError($DB->error);
    if($qry->num_rows>0){
        $info=$qry->fetch_assoc();
        $inn_qry=$DB->query("SELECT * FROM `".TAB_ASSIGN."` WHERE `type`='installation' AND `type_id`='{$info['install_service_id']}' AND `cust_id`='{$cid}'") or ThrowError($DB->error);
        if($inn_qry->num_rows>0){
            $DB->query("DELETE FROM `".TAB_ASSIGN."` WHERE `type`='installation' AND `type_id`='{$info['install_service_id']}' AND `cust_id`='{$cid}'") or ThrowError($DB->error);
        }
    ...
    /*$DB->query("DELETE FROM `" . TAB_INSTALL . "` WHERE `cust_id`='$cid' and `install_id`='$id'") or ThrowError($DB->error);
    if ($DB->affected_rows > 0) {
        $DB->query("DELETE FROM `".TAB_ASSIGN."` WHERE `cust_id`='$cid' AND `type_id`='$id' AND type='installation'") or ThrowError($DB->error);
        $result = array("status" => "ok", "result" => "You deleted AC ");
    } else {
        $result = array("status" => "no", "result" =>
...
$app->delete("/Customer/:cid/AMC/:id", function ($cid, $id) use ($app) {
    global $DB;
    
    $qry=$DB->query("SELECT * FROM `".TAB_AMC_SERVICE."` WHERE `cust_id`='{$cid}' and `amc_id`='{$id}'") or ThrowError($DB->error);
    if($qry->num_rows>0){
        while($info=$qry->fetch_array()){
            $info['amc_service_id'];
            $inn_qry=$DB->query("SELECT * FROM `".TAB_ASSIGN."` WHERE `cust_id`='{$cid}' AND `type`='amc' AND `type_id`='{$info['amc_service_id']}'") or ThrowError($DB->error);
            if($inn_qry->num_rows>0){
                $DB->query("DELETE FROM `".TAB_ASSIGN."` WHERE `cust_id`='{$cid}' AND `type`='amc' AND `type_id`='{$info['amc_service_id']}'") or ThrowError($DB->error);
            }
        }
    }
    $DB->query("DELETE FROM `".TAB_AMC."` WHERE `cust_id`='$cid' and `amc_id`='$id' ") or ThrowError($DB->error);
    if($DB->affected_rows>0){
    ...
    if ($DB->affected_rows > 0) {
        $DB->query("DELETE FROM `".TAB_ASSIGN."` WHERE `cust_id`='$cid' and `type_id`='$id' AND `type='amc'` ");
        $result = array("status" => "ok", "result" => "You deleted AMC ");
    } else {
        $result = array("status" => "no", "result" =>
            "Unable to delete. Please try again");
...
$app->get("/Customer/:cid/OTS", function ($cid) use ($app) {
    ...
    $qry = $DB->query("SELECT * FROM `" . TAB_OTS .
        ...
        "`) as ac_type ,(SELECT make_id,make as brand_name FROM `" . TAB_AC_MAKE .
        "`) as ac_make,`" . TAB_AC_LOCATION . "` as acloc INNER JOIN `" .TAB_CUSTOMER_AC . "` as cad,`" . TAB_AC_TONNAGE . "` as ac_ton
 ...
 and acloc.`ac_location_id`=cad.`ac_location`  and ac_type.`ac_type_id` = cad.`ac_type` and ac_ton.`tonnage_id`=cad.capacity and ots.`enabled`='1'");
    if ($qry->num_rows > 0) {
        ...
        while ($info = $qry->fetch_assoc()) {
            ...
            /** STARTS : CHECK STATUS OF THIS SERVICE */
            $check_assign = $DB->query("SELECT * FROM `".TAB_ASSIGN."` as assign
             WHERE assign.`type`='ots' AND assign.`type_id`={$info['ots_id']}") or ThrowError($DB->error);

            if ($check_assign->num_rows > 0) {
                while($check_data = $check_assign->fetch_assoc()) {
                    if ($check_data['status'] == 'c') {
                        $info['assignment_status'] = "Done";
                    } else {
                        $info['assignment_status'] = "Pending..";
            ...
            /** ENDS : CHECK STATUS OF THIS SERVICE */
...
$app->delete("/Customer/:cid/OTS/:id", function ($cid, $id) use ($app) {
    ...
    if ($DB->affected_rows > 0) {        
        $DB->query("DELETE FROM `".TAB_ASSIGN."` WHERE `cust_id`='$cid' and `type_id`='$id' AND `type`='ots'");
 ...
 and acloc.`ac_location_id`=cad.`ac_location`  and ac_type.`ac_type_id` = cad.`ac_type` and complaint.`enabled`='1' and pt.`ac_problem_id`=complaint.`problem_type`");
    if ($qry->num_rows > 0) {
        ...
        while ($info = $qry->fetch_assoc()) {
            ...
            /** STARTS : CHECK STATUS OF THIS SERVICE */
            $check_assign = $DB->query("SELECT * FROM  `".TAB_ASSIGN."` as assign
            WHERE assign.`type_id`='{$info['com_id']}' AND assign.`type`='complaint'") or ThrowError($DB->error);
...
$app->post("/Customer/:cid/Complaints", function ($cid) use ($app) {
    ...
    if ($cid > 0) {
        ...
        if (!empty($ac_id) && !empty($problem_type)) {
            ...
            $DB->query("INSERT INTO `" . TAB_COMPLAINT . "`(`cust_id`, `ac_id`,`problem_type`, `problem_desc`,`preferred_date`)
        VALUES('$cid','$ac_id','$problem_type','$problem_desc','$date')") or ThrowError($DB->error);
            if ($DB->affected_rows > 0) {
                $result['status'] = "ok";
                $result['result'] = "Successfully Added";
                $app->status(201);
...
$app->delete("/Customer/:cid/Complaints/:id", function ($cid, $id) use ($app) {
    ...
    if ($DB->affected_rows > 0) {
        $DB->query("DELETE FROM `".TAB_ASSIGN."`  WHERE `cust_id`='$cid' AND `type_id`='$id' AND `type`='complaint'");
            $result = array("status" => "ok", "result" => "Complaint deleted successfully...");
    } else {
        $result = array("status" => "no", "result" =>
            "Unable to delete. Please try again");
    }
...
$app->get("/Customer/:cid/Uninstalled", function ($cid) use ($app) {
    ...
    $check_ac = $DB->query("SELECT ac_id FROM `" . TAB_CUSTOMER_AC . "` as cust WHERE cust.`cust_id`='{$cid}' AND cust.ac_id NOT IN (SELECT `ac_id` as ac FROM `" . TAB_INSTALL . "` where `cust_id`='{$cid}')") or ThrowError($DB->error);
    $result = array();
    if ($check_ac->num_rows > 0) {
        $total = 0;
        while ($check_data = $check_ac->fetch_assoc()) {
            $qry = $DB->query("SELECT * FROM `" . TAB_CUSTOMER_AC . "` as cad
...
WHERE  cad.ac_id='{$check_data['ac_id']}'  and cad.`make`=ac_make.`make_id`
 and acloc.`ac_location_id`=cad.`ac_location`  and ac_type.`ac_type_id` = cad.`ac_type` and ac_ton.`tonnage_id`=cad.capacity  ") or
        ...
        if ($total > 0) {
            ...
            $result = array("status" => "no", "result" => "All AC are already added for installation");
        }
